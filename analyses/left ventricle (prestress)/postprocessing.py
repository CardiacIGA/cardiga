...
def main(nrefine : int, Usample : int, VTKboundaryOnly : bool, convert2Uniform: bool, saveTemporalVTK: bool, saveVideoVTK: bool, savePointvalues: bool):
    '''
    .. arguments::
    ...
    tstart = 0   # [ms]
    tend   = 800 # [ms]
    tref   = 2   # [ms] reference instance for green-lagrange strain 
    time   = numpy.linspace(tstart,tend,int(tend/2)) 
    
    

    ## Load topo, geom, ns, and lhs 
    topo, geom, ns = load_pickle(folder+'Leftventricle_spline{}_nref{}_space'.format(3,nrefine))
    LHS            = load_pickle(folder+'Leftventricle_spline{}_nref{}_result'.format(3,nrefine))  
    ...
    ns.Ffpost0_i = 'Fpost_ij ef_j'
    ns.ls0ref    = 'lsc0 ( Ffpost0_i Ffpost0_i )^0.5'
...
#    ns.EpostT_ij  = '0.5 ( FFpostT_ij - δ_ij )'
    ...
    if saveTemporalVTK:
      ...
      for i in idx: 
        ...
        Myofiber_strain = Ubezier.eval('ln( ls0ref / lsc0 )' @ ns, **LHS[i])
    ...
    if saveVideoVTK:
      ...
      for i in idx: 
        treelog.info("Index value: {}/{}, physical time: {}".format(i,idx[-1],dt*i))
        ...
        U, σff = Ubezier.eval(['u_i','stress_ij ef_i ef_j'] @ ns, **LHS[i])
        export.vtk('IGA_GPA_simulation_result_{}'.format(str(i*2).zfill(4)), Ubezierbase.tri, X, U=U, Fiber_stress=σff)      
    #------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if savePointvalues:
      ...
      idx    = range(int(tstart/dt),int(tend/dt)+1)
      #idx    = range(0, len(LHS)) 
      folder = os.path.join(folder,'point values (6 ncycle)/')
      ...
      for ikey, ivalue in Coordinates.items():
         for j, (jkey, jvalue) in enumerate(ivalue.items()):
            ...
            for k, i in enumerate(idx): 
              ...
              Myofiber_strain[k] = pointBezier.eval('ln( ls0ref / lsc0 )' @ ns, **LHS[i])    
