...
# from multimodels import Models
# from nutils import solver, function, mesh, cli, export, cache, types
# import numpy, treelog, pickle, math

# import matplotlib.pyplot as plt
# import matplotlib.collections

# #from activation_wall import active_stress
# from extract_results import get_sample_results, get_sampled_topo
# from extract_mesh import extract_mesh
# from solvers import my_solver


## Bi-ventricle simulation
# def main(save:int, continu:int, post:int):
#     '''
#     .. arguments::
...
def main(nrefine: int, dt: float, ncycle: int, TimeIntScheme: float, btype: str, bdegree: int, qdegree: int, saveVTK: bool, savePost: bool):
    ...
    ''' 
    ml=1e-6;mm=1e-3;mmHg=133.3223684;ms=1e-3;s=1;kPa=1e3;cm=1e-2;m=1

    ## Important notes:
    # - Number of tnelems should be 'correct'
    # - The local coordinate system origin should be located correctly
    # - No portruding walls   
    # - wallboundary names
    # - Disable RBM and use springs or null-space (option)
    # - Check fiber distribution
     
    directC    = os.path.realpath(os.path.dirname(__file__)) # Current directory
    direct     = 'geometries'  
    filename   = 'LV_GEOMETRY_PS.txt'
    Ventricle  = Geometry(filename, direct=direct)
    topo,geom  = Ventricle.get_topo_geom(nrefine=nrefine) # <-- Make sure tnelems is 'correct', of the same order as the global refinement
